...
bool SIM7600::sendATCommand(const char* cmdStr, int timeout, char* result, int maxChars) {
...
void SIM7600::initConfig() {
  ...
  sendATCompare("AT+CPMS=\"MT\",\"SM\", \"ME\"", 1000, 1, "OK");  // Read & store msgs in flash, write with SIM
  sendATCompare("AT+CMGD=0,2", 1000, 1, "OK");                    // delete already read messages
  while (sendATCompare("AT+CREG?", 2000, 2, "+CREG: 0,1", "+CREG: 0,5") == 0) {
...
bool SIM7600::sendSMS(const char* number, const char* msg) {
  ...
  bool result = sendATCompare(smsCmd, 3000, 1, "> ");  // sets the SMS mode to text
  char buffer[32] = { 0 };
  if (result) {
...
bool SIM7600::readSMS(int index, SMSStruct& smsData) {
  ...
  if (!sendATCommand(cmd, 1000, buffer, sizeof buffer))
    return false;
  char* context1;
  char* context2;
...
SIM7600::GPSStruct SIM7600::getGPSLocation(unsigned long timeout) {
  ...
  while ((millis() - startTime) < timeout) {
    ...
    if (char* location = strstr(responseBuffer, "+CGPSINFO: ") + 11) {
      memmove(responseBuffer, location, strlen(responseBuffer) - (location - responseBuffer) + 1);
    }
...
SIM7600::GPSStruct SIM7600::formatGPS(char* GPSBuffer) {
  ...
  gpsData.latitude = ((int)gpsData.latitude / 100) + (fmod(gpsData.latitude, 100) / 60);

  token = strtok(NULL, ",");
  if (strcmp(token, "S") == 0)
    gpsData.latitude = gpsData.latitude * -1;

  token = strtok(NULL, ",");
  gpsData.longitude = atof(token);
  gpsData.longitude = ((int)gpsData.longitude / 100) + (fmod(gpsData.longitude, 100) / 60);

  token = strtok(NULL, ",");
  if (strcmp(token, "W") == 0)
    gpsData.longitude = gpsData.longitude * -1;
...
void SIM7600::stopTTS() {
  sendImmediate("AT+CTTS=0");
}
